// dashboard-stats.js - COMPLETE FIXED VERSION WITH PROPER CHART UPDATES
...
function changeYear(year) {
    ...
    if (currentView === 'yearly') {
        ...
        if (year) {
            ...
            selectedMonth = '';
            // Clear charts when no year is selected
            clearCharts();
        }
    }
}
...
async function loadMonthsForYear(year) {
    try {
        ...
        if (data.success && data.months && data.months.length > 0) {
            ...
            console.log(`✅ Loaded ${monthsForYear.length} months for FY ${year}`);
            ...
            monthSelect.innerHTML = '<option value="">No months for this year</option>';
        }
    } catch (error) {
...
async function loadDashboardData() {
    try {
        ...
        if (data.success) {
            ...
            // CRITICAL: Clear existing charts safely
            clearCharts();
            
            // CRITICAL: Small delay to ensure canvas is cleared before creating new charts
            setTimeout(() => {
            ...
            // Update savings with actual investment amounts
            if (data.savings) {
                console.log('💰 Savings data received:', data.savings);
                updateSavings(data.savings);
            }
    ...
    try {
        ...
        if (data.success && data.data) {
            ...
            if (data.data.tax_analysis && data.data.tax_analysis.answers) {
                ...
                console.log(`📊 From tax_analysis - 80C: ₹${month_80c}, 80D: ₹${month_80d}, Refund: ₹${taxSaved}`);
            } else {
                // Fallback to old structure
                const investments = data.data.investments || {};
                const insurance = data.data.insurance || {};
                month_80c = (investments.ppf || 0) + (investments.elss || 0) + (investments.life_insurance || 0);
                month_80d = (insurance.self || 0) + (insurance.parents || 0);
            }
            ...
            setTimeout(() => {
                ...
                createSingleMonthCharts(data.data, month);
            }, 100);
...
function updateSavings(savings) {
    ...
    // Update 80C progress bar
    if (progress80C && invested80C) {
        const invested = savings.invested80C || 0;
        const percentage = Math.min((invested / MAX_80C) * 100, 100);
        
        progress80C.style.width = percentage + '%';
        invested80C.textContent = formatCurrency(invested);
        
        console.log(`📊 80C: ₹${invested} (${percentage.toFixed(1)}% of ₹1.5L)`);
    }
    
    // Update 80D progress bar
    if (progress80D && invested80D) {
        const invested = savings.invested80D || 0;
        const percentage = Math.min((invested / MAX_80D) * 100, 100);
        
        progress80D.style.width = percentage + '%';
        invested80D.textContent = formatCurrency(invested);
        
        console.log(`📊 80D: ₹${invested} (${percentage.toFixed(1)}% of ₹25k)`);
    }
}
...
function updateSavingsTip() {
    ...
    if (selectedYear && currentView === 'yearly') {
        // Get current invested amounts from the progress bars
        const invested80C = parseFloat(document.getElementById('invested80C')?.textContent.replace(/[₹,]/g, '') || 0);
        const invested80D = parseFloat(document.getElementById('invested80D')?.textContent.replace(/[₹,]/g, '') || 0);
        
        const remaining80C = Math.max(0, 150000 - invested80C);
        const remaining80D = Math.max(0, 25000 - invested80D);
        
        if (remaining80C > 0 && remaining80D > 0) {
            tipElement.innerHTML = `💡 FY ${selectedYear}: You can invest ₹${remaining80C.toLocaleString('en-IN')} more in 80C and ₹${remaining80D.toLocaleString('en-IN')} more in 80D to maximize tax savings!`;
        } else if (remaining80C > 0) {
            tipElement.innerHTML = `💡 FY ${selectedYear}: You can invest ₹${remaining80C.toLocaleString('en-IN')} more in 80C to maximize tax savings!`;
        } else if (remaining80D > 0) {
            tipElement.innerHTML = `💡 FY ${selectedYear}: You can invest ₹${remaining80D.toLocaleString('en-IN')} more in 80D to maximize tax savings!`;
        } else if (invested80C >= 150000 && invested80D >= 25000) {
            tipElement.innerHTML = `🎉 Great job! You've maxed out 80C and 80D for FY ${selectedYear}!`;
        } else {
            tipElement.innerHTML = `📅 Showing data for financial year ${selectedYear}`;
        }
    } else if (selectedMonth && currentView === 'monthly') {
        tipElement.innerHTML = `📆 Showing investment data for ${selectedMonth}`;
    } else if (selectedYear && currentView === 'monthly') {
        tipElement.innerHTML = `📅 Year selected: ${selectedYear}. Choose a month to see details.`;
    } else {
