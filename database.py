...
class Database:
    ...
    def _get_financial_year(self, month_key):
        ...
        try:
            ...
            if len(parts) == 2:
                ...
                # If month is Jan-Mar, it belongs to previous year's financial year
    ...
    def save_monthly_record(self, email, month_data):
        """Save monthly financial record for user with financial year"""
        data = self._load()
        ...
        # Calculate financial year (e.g., "April 2025" -> "2025-26")
        financial_year = self._get_financial_year(month_key)
        ...
        # Store monthly record with all details including financial year
        data["users"][email]["financial_data"][month_key] = {
            ...
            "financial_year": financial_year  # ADDED: Store financial year
        }
    ...
    # If financial_year specified, filter months by their financial year
        ...
        # Get financial year from data or calculate it
            month_fy = data.get('financial_year')
            if not month_fy:
                month_fy = self._get_financial_year(month_key)
        
        # If financial_year filter is applied, check if month belongs to that year
    ...
    def get_available_financial_years(self, email):
        ...
        # Try to get from stored data first
            ...
            # Calculate if not stored
                fy = self._get_financial_year(month_key)
                if fy:
                    years.add(fy)
